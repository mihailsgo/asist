import { createMockOrchestrator, formatDocType, parseRawData, statusLabels, statusStepIndex } from './app-core.js';
...
(() => {
  ...
  function updateBulkSelectionUI() {
    ...
    if (openBulkButton) {
      ...
      openBulkButton.textContent = count
        ? `Review & Sign Selected (${count})`
        : 'Review & Sign Selected';
  ...
  function updateDropzones() {
    ...
    if (outputDropSummaryEl) {
      outputDropSummaryEl.textContent = stagedCount
        ? `${stagedCount} package${stagedCount === 1 ? '' : 's'} staged for delivery`
        : 'Signed packages will appear here';
    }
